...
if __name__ == '__main__':
    ...
    model = ReconFormer(in_channels=2, out_channels=2, num_ch=(96, 48, 24),num_iter=5,
        ...
        use_checkpoint=(False, False, False, False, False, False)).to(device)
    
    # model = ReconFormer(in_channels=2, out_channels=2, num_ch=(12, 12, 12),num_iter=5,
    #     down_scales=(2,1,1.5), img_size=320, num_heads=(6,6,6), depths=(2,1,1),
    #     window_sizes=(8,8,8), mlp_ratio=2., resi_connection ='1conv',
    #     use_checkpoint=(False, False, False, False, False, False)).to(device)


    image = torch.randn((1, 2, 320, 320)).to(device)
    kspace = torch.randn((1, 2, 320, 320)).to(device)
    mask = torch.randn((1, 2, 320, 320)).to(device)

    out = model(image, kspace, mask)
