...
def forward_pass_varnet_pretrained_d5c5_lineval(batch, model, loss_function, args):

    kspace_und = batch.kspace_und.to(args.dv)
    mask = batch.mask.to(args.dv)
    num_low_freqs = batch.num_low_freqs.to(args.dv)

    _, out_d5c5 = model(masked_kspace=kspace_und,
                                            mask=mask,
                                            num_low_frequencies=num_low_freqs
                                            )
    
    tar = batch.target.to(args.dv)  # (b, c, h, w)
    loss = loss_function(tar, out_d5c5)

    return loss, out_d5c5


class Varnet_pretrained_D5C5_lineval(nn.Module):
    def __init__(
            self, num_chans=256
    ):
        super().__init__()
...
def train_():
    ...
    # LOAD MODEL
    model = Varnet_pretrained_D5C5_lineval(num_chans = args.num_chans)
    ...
    for _ in range(args.ne):
        ...
        with tqdm(train_loader, unit="batch") as train_epoch:
            ...
            for b in train_epoch:
                ...
                train_loss, _ = forward_pass_varnet_pretrained_d5c5_lineval(b, model, loss_fn, args)
        ...
        with torch.no_grad():
            ...
            with tqdm(val_loader, unit="batch") as val_epoch:
                for b in val_epoch:
                    ...
                    val_loss, x_hat = forward_pass_varnet_pretrained_d5c5_lineval(b, model, loss_fn, args)
            ...
            if m.best_epoch or (m.epoch_count % args.pf == 0):
                with tqdm(viz_loader, unit="batch") as viz_epoch:
                    for b in viz_epoch:
                        ...
                        _, x_hat = forward_pass_varnet_pretrained_d5c5_lineval(b, model, loss_fn, args)
