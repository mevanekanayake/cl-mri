...
def forward_pass_miccan(batch, model, loss_function, args):

    image = batch.image_zf2.to(args.dv)  # (b, n_coils, h, w, c=2)
    image, b_image = chans_to_batch_dim(image) # (b*n_coils, 1, h, w, c=2)
    image = image.squeeze(1).permute(0, 3, 1, 2) # (b*n_coils, c=2, h, w)

    kspace_und = batch.kspace_und.to(args.dv) # (b, n_coils, h, w, c=2)
    kspace_und, b_kspace = chans_to_batch_dim(kspace_und) # (b*n_coils, 1, h, w, c=2)
    kspace_und = kspace_und.squeeze(1).permute(0, 3, 1, 2) # (b*n_coils, h, w, c=2)
    
    mask = batch.mask.to(args.dv).to(torch.float) # (b, n_coils, h, w, c=2)
    mask, b_mask = chans_to_batch_dim(mask) # (b*n_coils, 1, h, w, c=2)
    mask = mask.squeeze(1).permute(0, 3, 1, 2) # (b*n_coils, c=2, h, w)

    out = model(image, kspace_und, mask)

    out = complex_abs(out.permute(0, 2, 3, 1)).unsqueeze(-1)
    out = out.unsqueeze(1)
    out = batch_chans_to_chan_dim(out, b_image).permute(0, 1, 4, 2, 3) # (b, n_coils, c=1, h, w)
    ...
    for _ in range(args.ne):
        ...
        with tqdm(train_loader, unit="batch") as train_epoch:
            ...
            for b in train_epoch:
                ...
                train_loss, _ = forward_pass_miccan(b, model, loss_fn, args)
        ...
        with torch.no_grad():
            ...
            with tqdm(val_loader, unit="batch") as val_epoch:
                for b in val_epoch:
                    ...
                    val_loss, x_hat = forward_pass_miccan(b, model, loss_fn, args)
            ...
            if m.best_epoch or (m.epoch_count % args.pf == 0):
                with tqdm(viz_loader, unit="batch") as viz_epoch:
                    for b in viz_epoch:
                        ...
                        _, x_hat = forward_pass_miccan(b, model, loss_fn, args)
