...
def forward_pass_varnet_pretrained_reconformer_lineval(batch, model, loss_function, args):

    kspace_und = batch.kspace_und.to(args.dv)
    mask = batch.mask.to(args.dv)
    num_low_freqs = batch.num_low_freqs.to(args.dv)

    _, out_reconformer = model(masked_kspace=kspace_und,
                                            mask=mask,
                                            num_low_frequencies=num_low_freqs
                                            )
    
    tar = batch.target.to(args.dv)  # (b, c, h, w)
    loss = loss_function(tar, out_reconformer)

    return loss, out_reconformer


class Varnet_pretrained_ReconFormer_lineval(nn.Module):
    def __init__(
            self
    ):
        super().__init__()
        ...
        self.reconformer = ReconFormer(in_channels=2, out_channels=2, num_ch=(96, 48, 24),num_iter=5,
        ...
        self.reconformer.load_state_dict(torch.load('F_X8_checkpoint.pth'))
    ...
    def forward(self, masked_kspace, mask, num_low_frequencies):
        ...
        image_zf2 = ifft2c(masked_kspace) # (b, n_coils, h, w, c=2)
        image_zf = complex_abs(image_zf2) # (b, n_coils, h, w)
        image = rss(image_zf, dim=1).unsqueeze(1) # (b, c=1, h, w)
        std = image.mean(dim=(1,2,3)).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
        image = image/std
        image2 = torch.cat((image, torch.zeros_like(image)), dim=1) # (b, c=2, h, w)
        kspace_und = fft2c(image2.permute(0, 2, 3, 1)).permute(0, 3, 1, 2) # (b, c=2, h, w)
        
   

        clr = self.varnet_pretrained(masked_kspace, mask, num_low_frequencies) # (b, n_coils, h, w, c=2)
        clr = complex_abs(clr).unsqueeze(-1)    # (b, n_coils, h, w, c=1)
        clr = rss(clr, dim=1).permute(0, 3, 1, 2)  # (b, c=1, h, w)
        clr_n = clr/std
        clr2 = torch.cat((clr_n, torch.zeros_like(clr_n)), dim=1) # (b, c=2, h, w)

        mask = mask[:,0,:,:,:].float().permute(0, 3, 1, 2)

        out2 = self.reconformer(clr2,
                    kspace_und,
                    mask)

        out2 = complex_abs(out2.permute(0, 2, 3, 1)) # (b, h, w)
        out2 = out2.unsqueeze(1) # (b, c=1, h, w)
        out2 = out2*std
...
def train_():
    ...
    if args.ckpt is None and args.pret is not None:
        ...
        raise ValueError('D5C5 checkpoint weights and pretrained VarNet weights both cannot be None or loaded at the same time!')
    ...
    # LOAD MODEL
    model = Varnet_pretrained_ReconFormer_lineval()
    ...
    for _ in range(args.ne):
        ...
        with tqdm(train_loader, unit="batch") as train_epoch:
            ...
            for b in train_epoch:
                ...
                train_loss, _ = forward_pass_varnet_pretrained_reconformer_lineval(b, model, loss_fn, args)
        ...
        with torch.no_grad():
            ...
            with tqdm(val_loader, unit="batch") as val_epoch:
                for b in val_epoch:
                    ...
                    val_loss, x_hat = forward_pass_varnet_pretrained_reconformer_lineval(b, model, loss_fn, args)
            ...
            if m.best_epoch or (m.epoch_count % args.pf == 0):
                with tqdm(viz_loader, unit="batch") as viz_epoch:
                    for b in viz_epoch:
                        ...
                        _, x_hat = forward_pass_varnet_pretrained_reconformer_lineval(b, model, loss_fn, args)
