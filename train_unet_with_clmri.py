...
def forward_pass_varnet_pretrained_unet_lineval(batch, model, loss_function, args):

    kspace_und = batch.kspace_und.to(args.dv)
    mask = batch.mask.to(args.dv)
    num_low_freqs = batch.num_low_freqs.to(args.dv)

    _, out_unet = model(masked_kspace=kspace_und,
                                            mask=mask,
                                            num_low_frequencies=num_low_freqs
                                            )
    
    tar = batch.target.to(args.dv)  # (b, c, h, w)
    loss = loss_function(tar, out_unet)

    return loss, out_unet


class Varnet_pretrained_Unet_lineval(nn.Module):
    def __init__(
            self, num_chans=256
    ):
        super().__init__()
    ...
    def forward(self, masked_kspace, mask, num_low_frequencies):
        ...
        out1 = self.varnet_pretrained(masked_kspace, mask, num_low_frequencies) # (b, n_coils, h, w, c=2)
        out1 = complex_abs(out1).unsqueeze(-1)    # (b, n_coils, h, w, c=1)
        out1_rss = rss(out1, dim=1).permute(0, 3, 1, 2)  # (b, c=1, h, w)
        out1, b = chans_to_batch_dim(out1)   # (b*n_coils, 1, h, w, c=1)
        out1 = out1.squeeze(1).permute(0, 3, 1, 2)    # (b*n_coils, c=1, h, w)
        
        mean = torch.mean(out1, dim=(1,2,3)).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1) # (b*n_coils, 1, 1, 1)
        std = torch.std(out1, dim=(1,2,3)).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1) # (b*n_coils, 1, 1, 1)
        out1_n = (out1 - mean) / std

        out2_n = self.unet(image=out1_n)    # (b*n_coils, c=1, h, w)
        out2 = out2_n * std + mean

        out2 = out2.permute(0, 2, 3, 1).unsqueeze(1) # (b*n_coils, 1, h, w, c=1)
        out2 = batch_chans_to_chan_dim(out2, b).permute(0, 1, 4, 2, 3) # (b, n_coils, c=1, h, w)
...
def train_():
    ...
    if args.ckpt is None and args.pret is not None:
        ...
        raise ValueError('Unet checkpoint weights and pretrained VarNet weights both cannot be None or loaded at the same time!')
    ...
    # LOAD MODEL
    model = Varnet_pretrained_Unet_lineval(num_chans = args.num_chans)
    ...
    for _ in range(args.ne):
        ...
        with tqdm(train_loader, unit="batch") as train_epoch:
            ...
            for b in train_epoch:
                ...
                train_loss, _ = forward_pass_varnet_pretrained_unet_lineval(b, model, loss_fn, args)
        ...
        with torch.no_grad():
            ...
            with tqdm(val_loader, unit="batch") as val_epoch:
                for b in val_epoch:
                    ...
                    val_loss, x_hat = forward_pass_varnet_pretrained_unet_lineval(b, model, loss_fn, args)
            ...
            if m.best_epoch or (m.epoch_count % args.pf == 0):
                with tqdm(viz_loader, unit="batch") as viz_epoch:
                    for b in viz_epoch:
                        ...
                        _, x_hat = forward_pass_varnet_pretrained_unet_lineval(b, model, loss_fn, args)
