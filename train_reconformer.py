...
def forward_pass_reconformer(batch, model, loss_function, args):

    image = rss(batch.image_zf.to(args.dv), dim=1) # (b, c=1, h, w)
    ...
    mask = batch.mask.to(args.dv).to(torch.float)[:,0,:,:,:].permute(0, 3, 1, 2)

    out = model(image2,
                  kspace_und,
                  mask)

    out = complex_abs(out.permute(0, 2, 3, 1)) # (b, h, w)
    out = out.unsqueeze(1) # (b, c=1, h, w)
    out = out*std
...
def train_():
    ...
    # LOAD MODEL
    model = ReconFormer(in_channels=2, out_channels=2, num_ch=(96, 48, 24),num_iter=5,
    ...
    for _ in range(args.ne):
        ...
        with tqdm(train_loader, unit="batch") as train_epoch:
            ...
            for b in train_epoch:
                ...
                train_loss, _ = forward_pass_reconformer(b, model, loss_fn, args)
        ...
        with torch.no_grad():
            ...
            with tqdm(val_loader, unit="batch") as val_epoch:
                for b in val_epoch:
                    ...
                    val_loss, x_hat = forward_pass_reconformer(b, model, loss_fn, args)

                    # END VALIDATION STEP
                    val_epoch.set_postfix(val_loss=val_loss.detach().item())
                    m.end_val_step(b.fname, b.slice_num, b.sequence, b.image_zf, x_hat.to('cpu'), b.target, val_loss.to('cpu'), b.max_val)
            ...
            if m.best_epoch or (m.epoch_count % args.pf == 0):
                with tqdm(viz_loader, unit="batch") as viz_epoch:
                    for b in viz_epoch:
                        ...
                        _, x_hat = forward_pass_reconformer(b, model, loss_fn, args)
